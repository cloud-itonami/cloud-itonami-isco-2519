(ns softdev.actor
  "SoftwareDevNECActor — the ISCO-08 2519 community softdev actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one softdev operation request
  ...
  The unconditional invariant: the SoftwareDevNECAdvisor can never
  directly commit a record the SoftwareDevNECGovernor refuses —
  every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [softdev.advisor :as advisor]
            [softdev.governor :as governor]
            [softdev.store :as store]))

(defn build-graph
  "Build a compiled SoftwareDevNECActor graph. `store` implements
  `softdev.store/Store`. `advisor` implements
  `softdev.advisor/Advisor` (defaults to `mock-advisor`).
